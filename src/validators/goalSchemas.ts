...
export const PayoffGoalCreateSchema = z.object({
  ...
  monthly_contribution: z.string().regex(/^\d+(\.\d{2})?$/, 'Must be a decimal with 2 places').optional(),
...
export const SavingsGoalCreateSchema = z.object({
  ...
  current_value: z.string().regex(/^\d+(\.\d{2})?$/, 'Must be a decimal with 2 places').optional(),
